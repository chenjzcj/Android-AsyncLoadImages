...
public class ImageManager {
	...
	public static List<Bucket> loadAllBucketList(Context context) {
		...
		for (int i = 0; i < imageList.size(); i++) {
			...
			Thumbnails tempThumbnails = (Thumbnails) tempThumbnailsSet
					.get(String.valueOf(tempImage.get_id()));
			if (tempThumbnails != null) {
				tempImage.setThumbnails(tempThumbnails);
			}

			Bucket tempBucke = (Bucket) tempBucketSet.get(StringUtils
					.getBucketPath(tempImage.get_data(),
							tempImage.get_display_name()));
			if (tempBucke != null) {
				tempBucke.addImages(tempImage);
			} else {
				tempBucke = new Bucket();
				tempBucke.setName(tempImage.getBucket_display_name());
				tempBucke.setPath(StringUtils.getBucketPath(
						tempImage.get_data(), tempImage.get_display_name()));
				tempBucke.addImages(tempImage);
				// Log.i("loadAllBucketList",
				// StringUtils.getBucketPath(tempImage.get_data(),
				// tempImage.get_display_name()));
				// Log.i("loadAllBucketList", tempBucke.toString());
				tempBucketSet.put(StringUtils.getBucketPath(
						tempImage.get_data(), tempImage.get_display_name()),
						tempBucke);
			}
		}
	...
	public static Hashtable loadAllThumbnailsSet(Context context) {
		...
		if (cursor != null) {
			while (cursor.moveToNext()) {
				...
				tempThumbnailsSet.put(
						String.valueOf(cursor.getInt(imageIdColumn)),
						thumbnails);
